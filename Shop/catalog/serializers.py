...
from .models import (Category, Product, ProductImage, Seller,
                     Discount, OrderProducts, Order, Promocode, CashBack)
        ...
        for product in data.get('products'):
            ...
            if product.get('discount'):
                ...
                if delta.days <= 0:
                    ...
                    result_price +=\
                        (price * (100 - discount_percent) / 100) * count
                else:
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'status', 'total_sum',
            'delivery_address', 'delivery_notification_before',
            'products', 'promocode',
            'use_cashback',
        )
    ...
    def create(self, validated_data):
        ...
        if use_cashback:
            allow_to_pay_py_points = total_sum * (100 - cashback.percent) / 100

            if user.cashback_points >= allow_to_pay_py_points:
                total_sum -= allow_to_pay_py_points
                user.cashback_points -= allow_to_pay_py_points
            else:
        ...
        order = Order.objects.create(total_sum=total_sum, user=user,
                                     **validated_data)
